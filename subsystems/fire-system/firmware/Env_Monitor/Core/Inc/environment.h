/*
 * environment.h
 *
 * Created on: Jul 5, 2026
 * Author: Navid
 */

#ifndef INC_ENVIRONMENT_H_
#define INC_ENVIRONMENT_H_

#include <lcd_parallel.h>
#include "stm32f1xx_hal.h"
#include "main.h"
#include "rtc.h"

#define TARGET_TEMP         25.0f
#define TOLERANCE_PERCENT   0.20f

#define HIGH_TEMP_THRESHOLD (TARGET_TEMP * (1.0f + TOLERANCE_PERCENT))

#define MOVING_AVG_SAMPLES  10
#define ALARM_LOCK_DURATION_MS 5000

#define FAN_PORT            FAN_GPIO_Port
#define FAN_PIN             FAN_Pin

#define HEATER_PORT         HEATER_GPIO_Port
#define HEATER_PIN          HEATER_Pin

#define FIRE_1_PORT         FIRE_1_GPIO_Port
#define FIRE_1_PIN          FIRE_1_Pin

#define FIRE_2_PORT         FIRE_2_GPIO_Port
#define FIRE_2_PIN          FIRE_2_Pin

#define SPRINKLER_1_PORT    SPRINKLER_1_GPIO_Port
#define SPRINKLER_1_PIN     SPRINKLER_1_Pin

#define SPRINKLER_2_PORT    SPRINKLER_2_GPIO_Port
#define SPRINKLER_2_PIN     SPRINKLER_2_Pin

#define BUZZER_PORT         BUZZER_GPIO_Port
#define BUZZER_PIN          BUZZER_Pin

void ENV_Init(ADC_HandleTypeDef *hadc_ptr, UART_HandleTypeDef *huart_ptr, RTC_HandleTypeDef *hrtc_ptr);
void ENV_Task(void);
void ENV_ReceiveHandler(uint8_t rx_char);

#endif /* INC_ENVIRONMENT_H_ */
